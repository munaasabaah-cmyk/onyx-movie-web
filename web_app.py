# -*- coding: utf-8 -*-
"""ONYX CINEMA v5.0 — Full Modal + No Source Buttons + Wide Layout"""
...
def tmdb(ep, params=None):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ONYX/5.0"})
...
.mo.on{display:flex}
.mo-box{background:#181818;border:1px solid var(--border);border-radius:12px;padding:1.5rem;width:98%;max-width:1600px;position:relative;margin:auto;min-height:auto}
.mo-close{position:absolute;top:.8rem;left:.8rem;background:var(--surface2);color:#fff;width:40px;height:40px;border-radius:50%;font-size:1.1rem;z-index:100;cursor:pointer;transition:.2s;display:flex;align-items:center;justify-content:center}
.mo-close:hover{background:var(--accent)}

/* PLAYER — كبير وواضح */
.pcont{width:100%;aspect-ratio:16/9;background:#000;border-radius:8px;overflow:hidden;margin-bottom:1.2rem;box-shadow:0 10px 40px rgba(0,0,0,.8)}
.pcont iframe{width:100%;height:100%;border:none;display:block}

/* MOVIE HEADER — عريض */
.mh{display:flex;gap:2rem;flex-wrap:wrap;margin-bottom:1.5rem}
.mp{width:200px;flex-shrink:0;border-radius:8px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,.8)}
.mp img{width:100%}
.mi{flex:1;min-width:300px}
.mi h2{font-size:1.8rem;margin-bottom:.6rem;font-weight:800;line-height:1.2}
...
.mm .lang{background:var(--surface2);color:#fff;padding:.2rem .6rem;border-radius:4px;font-weight:700;font-size:.78rem}
.mg{color:var(--text2);font-size:.88rem;margin-bottom:1rem}
.md{color:#ccc;line-height:1.75;font-size:.92rem;margin-bottom:1.3rem;max-width:900px}
.btn-watch{background:var(--accent);color:#fff;padding:.85rem 2.2rem;border-radius:8px;font-size:1rem;font-weight:700;transition:.2s;cursor:pointer;display:inline-flex;align-items:center;gap:.5rem}
...
.sbtn{background:var(--surface);border:1px solid var(--border);color:var(--text2);padding:.5rem 1.1rem;border-radius:6px;font-size:.85rem;font-weight:700;cursor:pointer;transition:.2s}
...
.ecard:hover{transform:translateY(-4px);border-color:var(--accent);box-shadow:0 10px 30px rgba(229,9,20,.2)}
...
<div class="mo" id="profileMo" onclick="if(event.target.id==='profileMo')closeMo('profileMo')">
  <div class="mo-box" style="max-width:600px">
    ...
    <div class="prof-hd">
      ...
      <div>
        <h3 id="pName">...</h3>
        <p style="color:var(--text2);font-size:.82rem" id="pEmail">...</p>
      </div>
    </div>
    <div class="prof-tabs">
...
function loadGenres() {
  document.getElementById('genreRow').innerHTML = GENRES.map(g =>
    '<button style="background:var(--surface);border:1px solid var(--border);color:var(--text2);padding:.5rem 1.2rem;border-radius:20px;font-size:.82rem;font-weight:600;cursor:pointer;transition:.2s" onclick="filterGenre(' + g.id + ',\'' + g.name + '\')" onmouseover="this.style.borderColor=\'var(--accent)\';this.style.color=\'var(--accent)\'" onmouseout="this.style.borderColor=\'\';this.style.color=\'\'">' + g.name + '</button>'
  ).join('');
}
...
/* OPEN MOVIE — مع فيديو مباشر + بدون أزرار مصادر */
async function openMovie(id, type) {
  ...
  try {
    ...
    // PLAYER (فيديو مباشر — بدون أزرار مصادر)
    ...
    const cmts = S.comments ? (S.comments[ckey] || []) : [];
...
function startStream(idx, season, episode) {
  ...
  if (!frame) { console.warn('No player frame'); return; }
  const url = '/player?type=' + S.currentMovie.type + 
              '&id=' + S.currentMovie.id + 
              '&source=' + idx + 
              '&season=' + (season || 1) + 
              '&episode=' + (episode || 1);
  console.log('[ONYX] Loading:', url);
  frame.src = url;
...
if (src) {
  ...
  console.log('[ONYX Player] Loading:', url);
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "service": "ONYX CINEMA v5.0", "tmdb": "ok" if TMDB_API_KEY else "missing", "sources": len(PLAYER_SOURCES)})
