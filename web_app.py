# -*- coding: utf-8 -*-
"""🎬 ONYX MOVIE — Flask + TMDB + 25 Player Sources"""
...
# تحويل إلى JSON لتمريره للـ JavaScript
...
.nav-search input { background: transparent; border: none; outline: none; color: var(--text); padding: 0.55rem 1rem; width: 240px; font-size: 0.85rem; }
.nav-search input::placeholder { color: var(--text3); }
.nav-search button { background: var(--accent); color: #fff; padding: 0.55rem 1rem; font-size: 0.85rem; font-weight: 700; }
...
.modal-close { position: absolute; top: 1rem; left: 1rem; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.15); color: var(--text2); width: 40px; height: 40px; border-radius: 50%; font-size: 1.2rem; z-index: 10; }
...
.player-status { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; padding: 0.75rem 1rem; background: var(--surface); border-radius: 8px; border: 1px solid var(--border); flex-wrap: wrap; gap: 0.5rem; }
...
.player-tab { background: var(--bg3); border: 1px solid var(--border); color: var(--text2); padding: 0.4rem 0.8rem; border-radius: 6px; font-size: 0.72rem; font-weight: 600; white-space: nowrap; display: inline-flex; align-items: center; gap: 0.3rem; }
...
.season-btn { background: var(--surface); border: 1px solid var(--border); color: var(--text2); padding: 0.4rem 0.9rem; border-radius: 6px; font-size: 0.85rem; }
...
.episode-btn { background: var(--surface); border: 1px solid var(--border); color: var(--text2); padding: 0.75rem 1rem; border-radius: 8px; font-size: 0.85rem; text-align: right; }
...
function movieCard(m) {
  ...
  const poster = m.poster_path ? IMG + '/w500' + m.poster_path : 'https://via.placeholder.com/300x450/161d30/5a6480?text=🎬';
...
function renderHero(i) {
  ...
  document.getElementById('heroMeta').innerHTML = '<span>' + year + '</span><span class="rating">⭐ ' + rating + '</span><span>' + (m.media_type === 'tv' ? '📺 مسلسل' : '🎬 فيلم') + '</span>';
...
async function loadTopRated() {
  ...
  try {
    ...
    el.innerHTML = results.map((m, i) => {
      ...
      return '<div class="top-item" onclick="openMovie(' + m.id + ', \'movie\')"><div class="top-rank">' + String(i + 1).padStart(2, '0') + '</div><img class="top-thumb" src="' + poster + '"><div class="top-details"><div class="top-title">' + title + '</div><div class="top-meta"><span>' + year + '</span><span class="rating">⭐ ' + rating + '</span></div></div></div>';
    }).join('');
  } catch (e) { el.innerHTML = '<div class="loading">❌</div>'; }
}
  ...
  try {
    ...
    html += '<div class="player-status">' +
      '<div class="player-status-info">' +
        ...
        '<span style="color:var(--text2);font-size:0.8rem" id="statusName">' + SOURCES[0].name + '</span>' +
      '</div>' +
