# -*- -*- coding: utf-8 -*-
"""ONYX MOVIE — Flask + TMDB + Search Fixed + Clean UI"""
...
PLAYER_SOURCES = [
    {"name": "VidLink EN Default", "quality": "4K", "ads": "none",
     "movie": "https://vidlink.pro/movie/{id}?primaryColor=e50914&autoplay=true&audio=en",
     "tv":    "https://vidlink.pro/tv/{id}/{season}/{episode}?primaryColor=e50914&autoplay=true&audio=en"}
]
...
async function doSearch() {
  ...
  const title = document.getElementById('titleTrending');
  el.innerHTML = '<div class="loading"><div class="spinner"></div></div>';
  title.textContent = (currentLang === 'ar' ? 'نتائج البحث: ' : 'Search Results for: ') + q;
  document.getElementById('trending-section').scrollIntoView({ behavior: 'smooth' });
  try {
    const res = await fetch('/api/search?q=' + encodeURIComponent(q) + '&lang=' + currentLang);
    const data = await res.json();
    const results = (data.results || []).filter(m => m.poster_path);
    el.innerHTML = results.map(movieCard).join('') || '<div class="loading">' + (currentLang === 'ar' ? 'لا توجد نتائج' : 'No results found') + '</div>';
    document.getElementById('trendingCount').textContent = results.length;
  } catch (e) { el.innerHTML = '<div class="loading">Error searching</div>'; }
}
...
# ══════════════════════════════════════════════════════════
# ROUTES (مع تفعيل مسار البحث كامل /api/search)
# ══════════════════════════════════════════════════════════
