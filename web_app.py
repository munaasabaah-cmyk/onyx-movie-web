# -*- coding: utf-8 -*-
"""
ONYX CINEMA v4.0 — Netflix-Style + Responsive + Secure
Flask + TMDB + 20 Sources + Auth + Matches + AI Chat
"""
...
:root{
  --bg:#141414;--bg2:#181818;--surface:#242424;--surface2:#2f2f2f;
  --accent:#e50914;--gold:#f5c518;--green:#22c55e;
...
.btn-play{background:#fff;color:#000;padding:.7rem 1.8rem;border-radius:6px;font-size:.95rem;font-weight:700;transition:.2s;display:inline-flex;align-items:center;gap:.5rem;cursor:pointer}
...
.btn-watch{background:var(--accent);color:#fff;padding:.7rem 1.8rem;border-radius:6px;font-size:.95rem;font-weight:700;display:inline-flex;align-items:center;gap:.5rem;transition:.2s;cursor:pointer}
...
.oauth-btn{flex:1;background:var(--surface);border:1px solid var(--border);color:#fff;padding:.6rem;border-radius:6px;font-size:.82rem;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.5rem;transition:.2s}
...
#toast{position:fixed;bottom:1.8rem;left:50%;transform:translateX(-50%) translateY(80px);background:var(--surface2);border:1px solid var(--border);border-radius:50px;padding:.55rem 1.3rem;font-size:.82rem;font-weight:600;color:#fff;box-shadow:0 8px 30px rgba(0,0,0,.6);z-index:99999;transition:.4s;white-space:nowrap;max-width:90vw}
...
@media(max-width:900px){
  ...
  .nav-links.mobile a{padding:.7rem 1rem;font-size:.9rem}
  ...
  .hero-desc{font-size:.83rem;-webkit-line-clamp:2}
  .mo-box{padding:1.1rem}
  .egrid{grid-template-columns:repeat(auto-fill,minmax(160px,1fr))}
}
@media(max-width:500px){
  ...
  .section{padding:1rem 2%}
  .mh{gap:1rem}
  .mp{width:110px}
  .mi h2{font-size:1.15rem}
  .mi h2 + .mm{font-size:.75rem}
  .md{font-size:.83rem}
  .btn-watch{padding:.6rem 1.3rem;font-size:.85rem}
}
</style>
</head><body>
...
<section class="section" id="movies-sec">
  <div class="sec-hd"><h2 class="sec-title">الأكثر رواجاً</h2><span id="trendingCount" style="font-size:.8rem;color:var(--text2)"></span></div>
  <div class="row" id="trendingRow"><div class="loading"><div class="spinner"></div></div></div>
</section>

<!-- POPULAR -->
<section class="section">
  <div class="sec-hd"><h2 class="sec-title">الأفلام الشائعة</h2><span id="popularCount" style="font-size:.8rem;color:var(--text2)"></span></div>
...
<section class="section" id="series-sec">
  <div class="sec-hd"><h2 class="sec-title">المسلسلات</h2><span id="seriesCount" style="font-size:.8rem;color:var(--text2)"></span></div>
...
<div class="mo" id="profileMo" onclick="if(event.target.id==='profileMo')closeMo('profileMo')">
  <div class="mo-box" style="max-width:600px">
    ...
    <div class="prof-hd">
      ...
      <div>
        <h3 id="pName" style="font-size:1.1rem">...</h3>
        <p style="color:var(--text2);font-size:.8rem" id="pEmail">...</p>
        <p style="color:var(--text3);font-size:.75rem" id="pBio"></p>
      </div>
    </div>
    <div class="prof-tabs">
...
<!-- ═══════════════════════════════════════════════════════════
     MODAL: SEARCH
     ═══════════════════════════════════════════════════════════ -->
<div class="mo" id="searchMo" onclick="if(event.target.id==='searchMo')closeMo('searchMo')">
  <div class="mo-box" style="max-width:700px">
    <button class="mo-close" onclick="closeMo('searchMo')">✕</button>
    <h2 style="margin-bottom:1rem;font-size:1.2rem">نتائج البحث</h2>
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port, debug=False)<!-- TOAST -->
<div id="toast"></div>
...
<script>
/* ═══════════════════════════════════════════════════════════
   ONYX CINEMA v4.0 — Main JavaScript
   ═══════════════════════════════════════════════════════════ */
...
function setupEvents() {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      document.querySelectorAll('.mo.on').forEach(m => closeMo(m.id));
    }
  });
}
...
function movieCard(m) {
  ...
  const poster = m.poster_path
    ...
    : 'https://via.placeholder.com/300x450/242424/aaa?text=ONYX';
...
function nextSlide() {
  if (S.hero.length) setSlide((S.hIdx + 1) % S.hero.length);
}

function playHero() {
  const m = S.hero[S.hIdx];
...
async function loadTrending() {
  ...
  try {
    ...
    document.getElementById('trendingCount').textContent = res.length + ' عنصر';
...
async function loadPopular() {
  ...
  try {
    ...
    const d = await r.json();
    const res = (d.results || []).filter(m => m.poster_path).slice(0, 20);
    el.innerHTML = res.map(movieCard).join('');
    document.getElementById('popularCount').textContent = res.length + ' فيلم';
...
async function loadTopRated() {
  ...
  try {
    ...
    const d = await r.json();
    const res = (d.results || []).filter(m => m.poster_path).slice(0, 15);
    el.innerHTML = res.map(movieCard).join('');
  } catch (e) { el.innerHTML = '<div class="loading">❌</div>'; }
}
...
async function loadSeries() {
  ...
  try {
    ...
    const res = (d.results || []).filter(m => m.poster_path).slice(0, 20);
    el.innerHTML = res.map(movieCard).join('');
    document.getElementById('seriesCount').textContent = res.length + ' مسلسل';
  } catch (e) { el.innerHTML = '<div class="loading">❌</div>'; }
}
...
function loadGenres() {
  const el = document.getElementById('genreRow');
...
async function openMovie(id, type) {
  ...
  try {
    ...
    html += '<div class="pcont"><iframe id="pframe" allowfullscreen allow="autoplay; encrypted-media; fullscreen"></iframe></div>';
    ...
    // CAST
    if (cast.length) {
      html += '<div style="margin-top:1.2rem;border-top:1px solid var(--border);padding-top:1.2rem">';
      html += '<h3 style="margin-bottom:.9rem;font-size:1.05rem">طاقم التمثيل</h3>';
      html += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:.7rem">';
      cast.forEach(c => {
        const photo = c.profile_path ? IMG + '/w185' + c.profile_path : 'https://via.placeholder.com/150/2f2f2f/fff?text=?';
        html += '<div style="background:var(--surface);border-radius:6px;overflow:hidden;text-align:center;padding-bottom:.4rem">';
        html += '<img src="' + photo + '" style="width:100%;aspect-ratio:1/1;object-fit:cover">';
        html += '<div style="font-size:.72rem;font-weight:700;margin-top:.35rem;padding:0 .2rem">' + c.name + '</div>';
        html += '<div style="font-size:.62rem;color:var(--text2);padding:0 .2rem">' + (c.character || '') + '</div>';
        html += '</div>';
      });
      html += '</div></div>';
    }
    ...
    if (type === 'tv' && m.seasons && m.seasons.length) {
      ...
      html += '</div>';
      html += '<div class="egrid" id="epGrid"></div>';
      html += '</div>';
    }
    ...
    html += '<div class="cmt-sec">';
    html += '<h3>التعليقات (' + cmts.length + ')</h3>';
    ...
    html += '<div class="cmt-list" id="cmtList">' + renderComments(cmts) + '</div>';
    html += '</div>';

    ct.innerHTML = html;

    // Start streaming first source
    startStream(0);
...
function startStream(idx, season, episode) {
  ...
  if (!frame) return;
  frame.src = '/player?type=' + S.currentMovie.type + 
              '&id=' + S.currentMovie.id + 
              '&source=' + idx + 
              '&season=' + (season || 1) + 
              '&episode=' + (episode || 1);
}
  ...
  try {
    ...
    el.innerHTML = eps.map(ep => {
      const still = ep.still_path 
        ...
        : 'https://via.placeholder.com/300x169/242424/aaa?text=EP+' + ep.episode_number;
      return '<div class="ecard" onclick="startStream(0,' + seasonNum + ',' + ep.episode_number + ')">' +
        '<img class="ethumb" src="' + still + '" loading="lazy">' +
        '<div class="edet">' +
          '<div class="enum">حلقة ' + ep.episode_number + '</div>' +
          '<div class="etitle">' + (ep.name || '') + '</div>' +
        '</div>' +
      '</div>';
    }).join('');
  } catch (e) { el.innerHTML = '<div class="loading">❌</div>'; }
...
function renderComments(cmts) {
  if (!cmts.length) return '<p style="color:var(--text3);font-size:.82rem">لا تعليقات بعد</p>';
  return cmts.map(c => {
    const ava = c.ava || 'https://ui-avatars.com/api/?background=e50914&color=fff&size=80&name=' + encodeURIComponent(c.name);
    return '<div class="cmt">' +
      '<img class="cmt-ava" src="' + ava + '">' +
      '<div class="cmt-body">' +
        '<div class="cmt-head"><span class="cmt-name">' + c.name + '</span><span class="cmt-date">' + c.date + '</span></div>' +
...
function switchMo(a, b) {
  closeMo(a);
  setTimeout(() => openMo(b), 200);
}
...
function doRegister(e) {
  ...
  if (pass.length < 6) { toast('كلمة المرور قصيرة (6+)', 'err'); return; }
...
function simulateOAuth(provider) {
  ...
  if (!u) {
    u = {
      ...
      name: name,
      email: email,
      password: 'oauth_' + provider,
      bio: 'سجل عبر ' + provider,
...
function fillHist() {
  ...
  if (!S.hist.length) {
    el.innerHTML = '<p style="color:var(--text2);font-size:.82rem">لا يوجد سجل</p>';
    return;
  }
  el.innerHTML = S.hist.slice(0, 20).map(h => 
    '<div class="wl-item">' +
      '<div class="wl-info">' +
        '<div style="font-size:.82rem;font-weight:700">' + (h.title || '?') + '</div>' +
        '<div style="font-size:.68rem;color:var(--text3)">' + (h.date || '') + '</div>' +
      '</div>' +
    '</div>'
  ).join('');
}
...
async function fillFav() {
  ...
  if (!S.fav.length) {
    el.innerHTML = '<p style="color:var(--text2);font-size:.82rem">لا يوجد مفضلات</p>';
    return;
  }
  el.innerHTML = '<div class="loading"><div class="spinner"></div></div>';
  ...
  const items = await Promise.all(S.fav.map(async id => {
    try {
      ...
      const m = await r.json();
      const poster = m.poster_path ? IMG + '/w200' + m.poster_path : '';
      return '<div class="wl-item" onclick="closeMo(\'profileMo\');openMovie(' + id + ',\'movie\')" style="cursor:pointer">' +
        '<img class="wl-thumb" src="' + poster + '">' +
        '<div class="wl-info"><div style="font-size:.8rem;font-weight:700">' + (m.title || '') + '</div></div>' +
      '</div>';
    } catch (e) { return ''; }
  }));
...
function buildAdmin() {
  ...
  const all = [
    { name: 'Admin', email: OWNER.email, password: OWNER.password, isAdmin: true, joined: 'المالك' },
    ...S.users
  ];
...
function toggleMobileMenu() {
  ...
  links.classList.toggle('mobile', S.mobileMenuOpen);
  search.classList.toggle('mobile', S.mobileMenuOpen);
}
...
function goHome() {
  ...
  document.querySelector('.nav-links a').classList.add('on');
}

function scrollToSec(id) {
  document.getElementById(id + '-sec')?.scrollIntoView({ behavior: 'smooth' });
  document.querySelectorAll('.nav-links a').forEach(a => a.classList.remove('on'));
...
// LOG
// ══════════════════════════════════════════════════════════
console.log('[ONYX CINEMA] Loaded successfully');
console.log('[ONYX] Sources:', SOURCES.length);
</script>
